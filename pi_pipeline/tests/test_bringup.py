...
def test_steps_have_unique_ids_and_the_ordered_sections():
    ...
    assert sections == [
        "Assembly & mechanical (Phase 0)",
        "Serial link (Phase 0, BiBoard USB, no Pi)",
        "Assembly & mechanical (Phase 1)",
        "Serial link (Phase 5, via Pi)",
        "Voice (Phase 7, still on the stand)",
        "RL sim-to-real (Phase 6, still on the stand)",
...
def test_step_13_is_split_into_stand_and_floor():
    ids = [s.id for s in _steps()]
    assert "13a" in ids and "13b" in ids and "13" not in ids
    by_id = {s.id: s for s in _steps()}
    assert "STAND" in by_id["13a"].title.upper()
    assert "FLOOR" in by_id["13b"].title.upper()


def test_13b_confirm_gate_blocks_on_no(tmp_path, monkeypatch, capsys):
    from pi_pipeline.bringup import _run_interactive

    monkeypatch.setenv("G2_STATE_DIR", str(tmp_path))
    steps = [s for s in _steps() if s.id in ("13a", "13b")]
    answers = iter(["", "n"])   # do 13a, then decline the floor-test confirm
    monkeypatch.setattr("builtins.input", lambda *_: next(answers))
    _run_interactive(steps, set())
    assert _load_progress() == {"13a"}   # never got past the confirm to do 13b
    assert "go back to step 13a" in capsys.readouterr().out


def test_13b_confirm_gate_proceeds_on_yes(tmp_path, monkeypatch):
    from pi_pipeline.bringup import _run_interactive

    monkeypatch.setenv("G2_STATE_DIR", str(tmp_path))
    steps = [s for s in _steps() if s.id in ("13a", "13b")]
    answers = iter(["", "y", ""])   # do 13a, confirm the floor test, do 13b
    monkeypatch.setattr("builtins.input", lambda *_: next(answers))
    _run_interactive(steps, set())
    assert _load_progress() == {"13a", "13b"}
