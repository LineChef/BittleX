...
REPLANT_EVERY = 5
# NOTE: tried capping accumulated flexion tighter (18deg/3-cycle reset) to
# reduce the visible "squat" -- broke reliability outright (0/4, net
# progress never went positive). The crouch is generating real pull
# leverage, not just a cosmetic side effect -- reverted to the values that
# validated at 6/7 seeds. Isolating the jump->step fix on its own first.
...
for cyc in range(N_CYCLES):
    ...
    if this_pull > 0:
        for st in range(10):
            ...
            for _wait in range(6):
                p.setJointMotorControlArray(rid, env.joint_id, p.POSITION_CONTROL, lt, forces=_rear_force(1.0))
                sim_step()
    ...
    hip_idx, knee_idx = [(RB_HIP, RB_KNEE), (LB_HIP, LB_KNEE)][cyc % 2]
    lt2 = hold_targets.copy()
    for st in range(8):
        frac = (st + 1) / 8
        lt2 = hold_targets.copy()
        lt2[knee_idx] = hold_targets[knee_idx] - np.deg2rad(30) * frac
        tgt = _with_front_ik(lt2.copy(), fl_anchor, fr_anchor)
        for _wait in range(6):
            p.setJointMotorControlArray(rid, env.joint_id, p.POSITION_CONTROL, tgt, forces=_rear_force(1.0))
            sim_step()
    for st in range(8):
        frac = (st + 1) / 8
        lt3 = lt2.copy()
        lt3[hip_idx] = lt2[hip_idx] - np.deg2rad(18) * frac
        tgt = _with_front_ik(lt3.copy(), fl_anchor, fr_anchor)
        for _wait in range(6):
            p.setJointMotorControlArray(rid, env.joint_id, p.POSITION_CONTROL, tgt, forces=_rear_force(1.0))
            sim_step()
    lt4 = lt3.copy()
    lt4[knee_idx] = hold_targets[knee_idx]
    for _wait in range(10):
        tgt = _with_front_ik(lt4.copy(), fl_anchor, fr_anchor)
        p.setJointMotorControlArray(rid, env.joint_id, p.POSITION_CONTROL, tgt, forces=_rear_force(1.0))
        sim_step()
    hold_targets = _with_front_ik(lt4.copy(), fl_anchor, fr_anchor)
...
for _ in range(40):
    p.setJointMotorControlArray(rid, env.joint_id, p.POSITION_CONTROL, hold_targets, forces=np.ones(8) * ANCHOR_FORCE)
    sim_step()
