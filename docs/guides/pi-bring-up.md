...
| **Pi → BiBoard (serial, data-only)** | 3 lines: Pi **GPIO14 TXD → BiBoard RX2**, Pi **GPIO15 RXD → BiBoard TX2** (crossed), **GND↔GND**. The Pi's 5 V pin stays **unconnected** (PiSugar is sole power) — BiBoard's 5-pin header has two +5V pins, both also left unconnected. Confirmed pinout + annotated photos of exactly where to solder: [`hardware/biboard-pi-connector.md`](../hardware/biboard-pi-connector.md). | **female-to-female Dupont jumper wires** (get a 40-pack — you'll reuse them), and possibly a **5-pin header** to solder onto BiBoard if it's not already populated. **Check the Bittle X box first** — Petoi sometimes includes a BiBoard↔Pi cable, which would cover this already. |
...
#   --- power-saving (docs/hardware/pi-power.md; pi_pipeline.power.BOOT_CONFIG_LINES) ---
...
73.6 °C peak** under a 2-min stress (`docs/guides/gait-deployment.md`). Off saves ~2 g of
