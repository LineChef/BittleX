...
| 2026-09-03 | Probe batch R10 / R12a / R4 / R12b / R3 vs frozen `run20m_ppo` | (eval only) | **1 real gap: "stalls"** | 300+ rollouts, **1 fall** (a bad pick-up drop). Everywhere else the base *stops* rather than fights forward: carpet 90 s → 0.00 m/s by 60 s; side-hill 10–15° → ~0.025 m/s (−73 %); 15 mm thin raised lip → 0/16 crossed, recoils; stepped split → −92 % with a step under the **left** feet (−9 % under the right — lead-leg handedness). Pick-up/set-down → 31/32 re-acquire, **no training needed**. Report: "Gait Probe Report" artifact. → **R-NOSTALL** training designed (below). |
| 2026-09-03 | **R-NOSTALL** — "don't stall" training | **DEFERRED to the vision phase** | Dense reward: net fwd displacement over a ~1 s window (proj. on cmd heading), bleeds every stalled tick. Sparse bonus: +reward each ~0.15 m of net progress cleared *while resisted*. Capped below the fall penalty; real-move-cmd only; ~0.5 s grace. **Not run.** On the current blind, reactive, ±22°-residual policy this can only produce frantic scrabbling — it can't be *intentional* about an obstacle it can't perceive until a foot hits it, and the carpet lesson says a modest-gain fine-tune on the frozen base tends to erode more than it helps. The base already self-slows for stability as much as a blind reactive policy can (R12a). Right home: **Phase 8**, trained *together with* the direct forward terrain feature added to the policy observation — see `docs/project-plan.md` Phase 8 "DECIDED ARCHITECTURE (2026-09-03)". |
