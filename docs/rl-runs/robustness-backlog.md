...
| 2026-09-03 | **R-NOSTALL** — "don't stall" training | **IMPLEMENTED 2026-09-07** as `G2E_FAC_NOSTALL` (dense window-speed bleed + per-0.15 m breakthrough bonus while an obstacle is in view). First live use: Phase D vision-vs-blind A/B (`docs/rl-runs/vision-goal-locomotion-plan.md`) — fresh 20M, terrain feature ON, as the reward that gives vision leverage. Original design note kept: dense = net fwd displacement over ~1 s (proj. on cmd heading), bleeds every stalled tick; sparse = +reward each ~0.15 m cleared while resisted; real-move-cmd only. The "only makes sense trained WITH the forward terrain feature" call held — that's exactly the Phase D setup. |
