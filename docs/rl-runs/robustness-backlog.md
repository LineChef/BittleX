...
| 2026-09-03 | **R-NOSTALL** — "don't stall" training | *pending user go* | — | Dense reward: net fwd displacement over a ~1 s window (proj. on cmd heading), bleeds every stalled tick. Sparse bonus: +reward each ~0.15 m of net progress cleared *while resisted* (recent low speed / high foot-contact force). **Capped below the fall penalty**; fires only on a real move cmd; ~0.5 s grace. Terrain DR: carpet ~35 % + sustained side-hill ~20 % + thin raised lips @ random yaw ~25 % + split-steps both sides ~20 %, stacked, flat/slope/push DR kept. Run: continuation from `run20m_ppo`, `--finetune-lr 1e-4 --finetune-target-kl 0.05`, ~4 M steps, tag `run20m_nostall`. Gate: fwd progress up on all 4 stall terrains **and** flat cruise within ~5 % **and** decathlon falls ~0 % **and** no thrashing on replay GIFs — else revert. |
